...
from train_favar_v34 import load_windows_v33   # v34 도 같은 데이터 로딩 함수 사용

OUT_LOG  = RESULT / 'sign_alignment_v34_log.txt'
OUT_INFO = RESULT / 'sign_alignment_v34.json'
OUT_CSV  = RESULT / 'sign_alignment_v34.csv'
OUT_PLOT = PLOTS  / 'sign_alignment_v34.png'
...
CKPT_PATTERNS = [
    'favar_v34_*_best.pt',                              # v34 wavelet-PINF (전용)
    # v33 baseline 들 (paper ablation 용 비교)
    'favar_v33_sweep_K2_lam0p00_penrelu_best.pt',       # NLL only baseline
    'favar_v33_sweep_K2_lam0p50_penhinge_best.pt',      # Hinge λ=0.5 (Wavelet OFF)
    'favar_v33_sweep_K2_lam0p50_pentanh_best.pt',       # Tanh λ=0.5 (Wavelet OFF)
]
...
def main():
    ...
    X_tr, C_tr, _ = load_windows_v33(DATA / 'weekly_v34_train.csv', L=L0, stats=cond_stats)
    X_te, C_te, _ = load_windows_v33(DATA / 'weekly_v34_test.csv',  L=L0, stats=cond_stats)
