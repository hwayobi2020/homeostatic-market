"""1-step ahead Gaussian forecaster (vix included) — sp_return[t+1] 단일 예측.
...
ckpt prefix: m1step_v* — matrix multi-step (matrix_v*, mtxf_v*) 와 격리.
...
COLS_COND_BASE = ["tbill_wr", "m2_yoy_lag", "gdp_yoy_lag", "cpi_yoy_lag", "vix"]  # vix 추가 5ch
...
class CausalTransformer1Step(nn.Module):
    """Causal Transformer encoder + 1-step Gaussian head per target channel.

    Input:
      cond:        (B, total_len=53, d_cond)              past 52 + future 1 (tbill 활성)
      target_past: (B, past_len=52,  d_target)            past target

    Output:
      heads_out: dict {col: (mu, log_var)} where mu/log_var 는 (B,) — 1 step prediction
    """
    def __init__(self, d_cond, target_cols, d_model, n_heads, n_layers,
        ...
        # 분리 head — channel 별 nn.ModuleDict, 파라미터 완전 독립
    ...
    def forward(self, cond, target_past):
        ...
        # full target sequence — past observed, future 1 step masked (=0)
        ...
        # last position (future step = t+1) → heads
        h_t1 = h[:, -1, :]  # (B, d_model)

        outputs = {}
        for col in self.target_cols:
            out = self.heads[col](h_t1)  # (B, 2)
            mu = out[..., 0]               # (B,)
...
def run(spec, train_csv, val_csv, test_csv, save_dir, seed,
    ...
    tag_full = f"m1step_vix_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_seed{seed}"
    ...
    if best_state is not None:
        torch.save({
            ...
            "model_type":        "causal_transformer_1step",
            "cond_cols":         cols_cond,
...
def main():
    ...
    if args.fold is not None:
        ...
        train_csv = os.path.join(HERE, "data", "weekly_v33_vix_train.csv")
        val_csv   = None
        test_csv  = os.path.join(HERE, "data", "weekly_v33_vix_test.csv")
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(
            args.out_dir,
            f"m1step_vix_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_multiseed.csv")
