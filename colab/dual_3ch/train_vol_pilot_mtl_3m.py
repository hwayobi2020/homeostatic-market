"""Vol pilot MTL 3M — sp_return vol + aux vol 동시 학습 (3개월 horizon).

vol_pilot_mtl.py (1Y, hardcoded excess_liq aux) 의 3M (13w) 변종 + 다중 aux 옵션.

Variants (paper table 매핑):
  5: mtl_bp                 — aux=[bondpp_13w_lag],     cond=base 4ch         (paper #8)
  6: mtl_sp                 — aux=[stockpp_13w_lag],    cond=base 4ch         (paper #9)
  7: mtl_bp_sp              — aux=[bondpp + stockpp],   cond=base 4ch         (paper #10)
  8: mtl_el_bp_sp_cond      — aux=[excess_liq_yoy_lag], cond=base 4ch+bp+sp   (paper #11)
  9: mtl_el                 — aux=[excess_liq_yoy_lag], cond=base 4ch         (paper #12)

Targets:
  main: y_main = log( std(sp_return future 13w) )
  aux : y_aux_i = log( std(aux_target_i future 13w) )    per aux

Loss:
  L = MSE(pred_main, y_main) + λ × Σ_i MSE(pred_aux_i, y_aux_i)
  λ = 0.1 (1Y MTL 동일)

Architecture:
  Causal Transformer encoder (vol_pilot_v2 동일 backbone) — 분리 head per target
  Future portion (13 step) mean pool → 각 head 1 scalar

Selection: val_pearson_max on sp_return std level — 3M scalar trainer 와 동일
ckpt prefix: vol_pilot_mtl_3m_psel_v{ID}_{name}_pilot_seed{seed}
"""
...
# 3M (13w) horizon match — yoy_lag (52w) → 13w_cum_lag
COLS_COND_BASE = ["tbill_wr", "m2_13w_cum_lag", "gdp_13w_proxy_lag", "cpi_13w_cum_lag"]

VARIANTS_MTL = {
    5: dict(name="mtl_bp",
            aux_targets=["bondpp_13w_lag"],
            cond_extra=[]),
    6: dict(name="mtl_sp",
            aux_targets=["stockpp_13w_lag"],
            cond_extra=[]),
    7: dict(name="mtl_bp_sp",
            aux_targets=["bondpp_13w_lag", "stockpp_13w_lag"],
            cond_extra=[]),
    8: dict(name="mtl_el_bp_sp_cond",
            aux_targets=["excess_liq_yoy_lag"],
            cond_extra=["bondpp_13w_lag", "stockpp_13w_lag"]),
    9: dict(name="mtl_el",
            aux_targets=["excess_liq_yoy_lag"],
            cond_extra=[]),
}
...
def build_spec(variant_id):
    if variant_id not in VARIANTS_MTL:
        raise ValueError(f"variant must be in {list(VARIANTS_MTL)}, got {variant_id}")
    v = VARIANTS_MTL[variant_id]
    cols_cond = COLS_COND_BASE + v["cond_extra"]
    cols_target = ["sp_return"] + v["aux_targets"]
    mask_future_ch = list(range(1, len(cols_cond)))  # tbill (idx 0) only future activated
    return dict(
        variant_id=variant_id,
        name=v["name"],
        cols_cond=cols_cond,
        cols_target=cols_target,
        aux_targets=v["aux_targets"],
        mask_future_ch=mask_future_ch,
    )


# =====================================================================
# Model — shared encoder + 분리 head per target
# =====================================================================

class CausalTransformerVolMTL3M(nn.Module):
    """Shared Causal Transformer encoder + future mean pool + 분리 head per target.

    Input:
      cond:        (B, L=65, d_cond)
      target_past: (B, past_len=52, d_target)  past sp_return + past aux time series
    ...
    def __init__(self, d_cond, target_cols, d_model, n_heads, n_layers,
        ...
        self.total_len = past_len + future_len  # 65
...
# =====================================================================
# Data — multi-target log_std (3M, future 13w)
# =====================================================================
...
def run(spec, train_csv, val_csv, test_csv, save_dir, seed,
    ...
    aux_targets = spec["aux_targets"]
    mask_future = spec["mask_future_ch"]

    fold_str = f"_{fold_tag}" if fold_tag else ""
    sel_tag  = "_psel"  # pearson-based selection
    tag_full = f"vol_pilot_mtl_3m{sel_tag}_v{spec['variant_id']}_{spec['name']}{fold_str}_seed{seed}"
    ...
    print(f"\n{'='*72}")
    print(f"[Vol MTL 3M v{spec['variant_id']} = {spec['name']}{fold_str}] seed={seed}")
    print(f"  cond   = {cols_cond}  (D_COND={len(cols_cond)})")
    print(f"  target = {cols_target}  (main + {len(aux_targets)} aux)")
    print(f"  mask_future_ch = {mask_future}")
    print(f"  λ_aux = {lambda_aux},  L={L} (past={PAST_LEN} + future={FUTURE_LEN})")
    print(f"  selection_criterion = val_pearson_max (sp_return std level)")
    print(f"{'='*72}")
    ...
    model = CausalTransformerVolMTL3M(
    ...
    # Pearson-based selection (val Pearson on sp_return std level, higher = better)
    best_val_pearson = -float("inf")
    best_val_mse_sp_at_best = float("inf")
    best_state = None
    best_epoch = -1
    best_test_mse_sp = None
    best_test_r2_sp  = None
    best_test_pearson_sp = None
    best_test_spearman_sp = None
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, len(perm), batch):
            ...
            for col in aux_targets:
                ci = cols_target.index(col)
                aux_loss_list.append(((outputs[col] - yb[:, ci]) ** 2).mean())
            loss_aux_total = sum(aux_loss_list) if aux_loss_list else torch.tensor(0.0, device=device)
            loss = loss_main + lambda_aux * loss_aux_total
            opt.zero_grad()
            loss.backward()
            ...
            losses_aux_mean.append(float(loss_aux_total.item() / max(1, len(aux_targets))))

        train_loss      = float(np.mean(losses_total))
        train_loss_main = float(np.mean(losses_main))
        train_loss_aux  = float(np.mean(losses_aux_mean))
        ...
        val_aux_mean  = float(np.mean([val_mse[c]  for c in aux_targets]))
        test_aux_mean = float(np.mean([test_mse[c] for c in aux_targets]))
        ...
        print(f"  ep{epoch:>3d}  total={train_loss:.5f} (main={train_loss_main:.5f} aux_mean={train_loss_aux:.5f})  "
              f"val_p={val_p:+.3f}  val_sp={val_sp:.5f}  test_sp={test_sp:.5f} (R²={test_r2:+.3f})  "
              f"test_pearson={test_p:+.3f}")

        log.append(dict(epoch=epoch, train_loss=train_loss,
                        train_main=train_loss_main, train_aux_mean=train_loss_aux,
                        val_sp=val_sp, val_aux_mean=val_aux_mean, val_r2=val_r2,
                        val_pearson=val_p, val_spearman=val_s,
                        test_sp=test_sp, test_aux_mean=test_aux_mean, test_r2=test_r2,
                        test_pearson=test_p, test_spearman=test_s))

        # Pearson-based selection on val sp_return std level (higher is better)
        if not np.isfinite(val_p):
            pat += 1
        elif val_p > best_val_pearson + 1e-4:
            best_val_pearson = val_p
            best_val_mse_sp_at_best = val_sp
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            best_test_mse_sp = test_sp
            best_test_r2_sp  = test_r2
            best_test_pearson_sp = test_p
            best_test_spearman_sp = test_s
            best_test_aux_mse = test_aux_mean
    ...
    print(f"\n  best epoch {best_epoch}: val_pearson={best_val_pearson:+.4f}, val_sp_mse={best_val_mse_sp_at_best:.5f}")
    print(f"    test sp MSE       = {best_test_mse_sp:.5f}")
    print(f"    test sp R²        = {best_test_r2_sp:+.4f}")
    print(f"    test Pearson (sp) = {best_test_pearson_sp:+.4f}")
    print(f"    test Spearman(sp) = {best_test_spearman_sp:+.4f}")
    print(f"    test aux_mean MSE = {best_test_aux_mse:.5f}  (avg over {len(aux_targets)} aux)")
    ...
    summary = dict(
        ...
        val_sp_mse_at_best=best_val_mse_sp_at_best,
        test_mse=best_test_mse_sp,
        test_r2=best_test_r2_sp,
        test_pearson_std=best_test_pearson_sp,
        test_spearman_std=best_test_spearman_sp,
        test_aux_mse_mean=best_test_aux_mse,
        y_train_log_std_mean_sp=y_train_mean_sp,
        baseline_test_mse_sp=baseline_test_mse_sp,
        n_params=n_params,
        device=str(device),
...
def main():
    ap = argparse.ArgumentParser(description="Vol pilot MTL 3M — 13w future vol with multi-aux")
    ap.add_argument("--variant", type=int, default=9, choices=list(VARIANTS_MTL.keys()))
