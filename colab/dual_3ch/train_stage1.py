...
COLS_TARGET = ["excess_liq_wr"]   # weekly raw
...
MASK_FUTURE_CH_WITH_26W = [1, 2]   # tbill_26w_lag + excess_liq_wr in 3ch
...
def run(train_csv, test_csv, save_dir, seed=42, val_csv=None, fold_tag=None):
    torch.manual_seed(seed)
    np.random.seed(seed)
    fold_str  = f"_{fold_tag}" if fold_tag else ""
    tag_full  = f"stage1{fold_str}_seed{seed}"
    ...
    if os.path.exists(ckpt_path) and os.path.exists(summary_path_pre):
        ...
        with open(summary_path_pre) as f:
            return json.load(f)
    print(f"\n{'=' * 70}")
    print(f"[Stage1{fold_str}] seed={seed}")
...
def main():
    ...
    if len(results) > 1:
        ...
        df.to_csv(os.path.join(args.out_dir, f"stage1{fold_str}_multiseed_results.csv"), index=False)
