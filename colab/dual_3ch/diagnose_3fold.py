...
def main():
    print("=" * 72)
    print(" 5-fold holdout diagnostic")
    print("=" * 72)
    ...
    for fold in ["F0", "F1", "F2", "F3", "F4"]:
        diagnose_fold(fold)
    print()
    # Final aggregate (only if 5fold aggregate exists)
    agg = os.path.join(RESULT, "sensitivity_v13_summary_5fold_global.csv")
    if os.path.exists(agg):
        print(f"  5-fold aggregate CSV exists: {agg}")
    else:
        print(f"  5-fold aggregate CSV NOT yet — orchestrator did not finish all folds.")
