...
def main():
    print("#" * 96)
    print("# 진단: 시나리오 metab 진폭 vs 실제 위기 13주 metab 변동 (z-단위, 같은 척도)")
    print("#" * 96)

    for fold in FOLDS:
        # cond_stats (metab 표준화) — 체크포인트 meta 에서
        ...
        cmu = float(np.asarray(meta["cond_stats"]["mean"], float)[MI])
        csd = float(np.asarray(meta["cond_stats"]["std"], float)[MI])
        ...
        z_all = (df["metab_13w"].to_numpy(float) - cmu) / csd
        z_tr = (df_tr["metab_13w"].to_numpy(float) - cmu) / csd
        mb_rng = float(np.percentile(z_tr, 90) - np.percentile(z_tr, 10))   # 시나리오 진폭 기준(train)
        ramp_std = RAMP_SIG * mb_rng
        step_std = STEP_SIG * mb_rng

        # 실제 13주 metab 경로 std(z): 모든 origin 의 미래 13주 궤적 std
        path_std = np.array([z_all[t:t + FUTURE_LEN].std(ddof=0)
                             for t in range(len(z_all) - FUTURE_LEN + 1)])
        dates = df["date"].to_numpy()

        print(f"\n=== {fold} ===  (metab z: train μ={cmu:.4f} σ={csd:.4f}, p90-p10={mb_rng:.3f})")
        print(f"  [시나리오] 13주 metab 경로 std(z):  ramp(σ0.31)={ramp_std:.3f}   step(σ0.50)={step_std:.3f}")
    ...
    print("       level z(위기시 metab 수준)도 |z|>1.3(≈p90) 이면 → p10-p90 span 밖(꼬리/OOD).")
