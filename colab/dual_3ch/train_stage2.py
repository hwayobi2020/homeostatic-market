...
def run(seed, train_csv, test_csv, save_dir, stage1_dir,
    ...
    tag_full    = f"stage2{tag_norm_sr}{fold_str}_seed{seed}"
    ...
    if os.path.exists(ckpt_path) and os.path.exists(summary_path_pre):
        ...
        with open(summary_path_pre) as f:
            return json.load(f)
    print(f"\n{'=' * 70}")
    print(f"[Stage2{tag_norm_sr}{fold_str}] seed={seed}")
    if val_csv is not None:
        print(f"  val_csv = {val_csv}")
    print(f"  cond   = {COLS_COND}")
    print(f"  target = {COLS_TARGET}")
    stage1_filename = f"stage1{fold_str}_seed{seed}_best.pt"
    print(f"  Stage 1 ckpt = {stage1_filename} (paired, fold-matched)")
    ...
    # Load Stage 1 (paired seed, fold-matched)
    stage1_ckpt = os.path.join(stage1_dir, stage1_filename)
    ...
    # val_csv 처리
    if val_csv is not None:
        Xv, Cv_raw = load_window_arrays(val_csv, COLS_COND, COLS_TARGET, L=L)
        if Xv is None:
            print(f"[FAIL] val_csv too short")
            return None
        Cv_raw_mod = replace_future_with_stage1(Cv_raw, stage1_model, stage1_stats, PAST_LEN, device)
        Cv = zscore(Cv_raw_mod, stats_tr)
        if normalize_sp:
            Xv[..., SP_TARGET_IDX] = (Xv[..., SP_TARGET_IDX] - smu) / ssd
        Xtr_, Ctr_ = Xtr, Ctr
    ...
    summary = dict(
        stage=f"stage2{tag_norm_sr}{fold_str}",
        fold=fold_tag,
        seed=seed,
        normalize_sp=normalize_sp,
        cond_cols=COLS_COND,
...
def main():
    ...
    if len(results) > 1:
        ...
        df.to_csv(os.path.join(args.out_dir, f"stage2{tag_norm_sr}{fold_str}_multiseed_results.csv"), index=False)
