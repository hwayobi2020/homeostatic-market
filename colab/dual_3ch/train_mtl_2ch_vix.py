...
def run(train_csv, test_csv, save_dir, seed=42, normalize_vix=False,
    ...
    summary = dict(
        stage=f"mtl_2ch_vix{tag_liq}{tag_norm}{fold_str}",
        fold=fold_tag,
        seed=seed,
        no_liq=no_liq,
        normalize_sp=normalize_sp,
...
def main():
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(args.out_dir, f"mtl_2ch_vix{tag_liq}{tag_norm}{fold_str}_multiseed_results.csv")
        df.to_csv(out_csv, index=False)
        print(f"\n[MTL_2ch_vix{tag_liq}{tag_norm}{fold_str}] multi-seed (n={len(df)})")
