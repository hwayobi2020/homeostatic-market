...
def run(train_csv, test_csv, save_dir, seed=42, normalize_vix=False):
    ...
    print(f"\n{'=' * 70}")
    print(f"[MTL_2ch_vix{tag_norm}] seed={seed}")
    ...
    tag = f"mtl_2ch_vix{tag_norm}_seed{seed}"
...
def main():
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(args.out_dir, f"mtl_2ch_vix{tag_norm}_multiseed_results.csv")
        df.to_csv(out_csv, index=False)
        print(f"\n[MTL_2ch_vix{tag_norm}] multi-seed (n={len(df)})")
