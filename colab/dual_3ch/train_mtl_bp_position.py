...
COLS_COND_NO_26W   = ["tbill_wr"]   # 1ch (cond 최소화, MTL target 으로 bondpp 학습 유도)
COLS_COND_WITH_26W = ["tbill_wr", "tbill_26w_lag"]   # 2ch with 26w
COLS_COND = COLS_COND_NO_26W
MASK_FUTURE_CH_NO_26W   = []       # cond=[tbill] only, mask 없음 (tbill 시나리오 그대로)
MASK_FUTURE_CH_WITH_26W = [1]      # mask 26w
...
def main():
    ...
    ap.add_argument("--fold", default=None, choices=["F1", "F2", "F3"],
                    help="walk-forward fold. 주어지면 data/folds_v33/{fold}_{train,val,test}.csv 자동 매핑 + ckpt 에 _F{n} suffix")
