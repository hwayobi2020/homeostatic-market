...
CACHE_DIR = os.path.join(PS.RESULT_DIR, "debasement_cache")
os.makedirs(CACHE_DIR, exist_ok=True)

LOW_PCTL = 10                          # tbill 저금리 고정 수준
METAB_LEVELS = [10, 30, 50, 70, 90]    # [A] 미세 레벨 sweep
METAB_SHAPES = ["flat", "ramp_up", "ramp_down"]   # [B] 동적: 확대/축소 (build_shapes 재사용)
...
def main():
    ...
    for fold in FOLDS:
        ...
        # [B] 저금리에서 metab 동적 경로
        print(f"  [B] tbill=p{LOW_PCTL} 고정, metab 경로 (평균=p50·진폭=p90-p10)  "
              f"— skew / UWmean / UWcvar1 / term  (ramp_up=유동성 확대)")
        for name in METAB_SHAPES:
            sk = _agg([d["dynamic_lowrate"][name]["skew"] for d in loaded])
            uw = _agg([d["dynamic_lowrate"][name]["uw_mean"] for d in loaded])
            uc = _agg([d["dynamic_lowrate"][name]["uw_cvar1"] for d in loaded])
            tm = _agg([d["dynamic_lowrate"][name]["term_mean"] for d in loaded])
            print(f"    {name:<10}  skew {sk[0]:+.3f}±{sk[1]:.3f}  UWmean {uw[0]:+.4f}±{uw[1]:.4f}  "
                  f"UWcvar1 {uc[0]:+.4f}  term {tm[0]:+.4f}")

    print("\n[판정] [A] 저금리에서 metab 레벨↑ → skew 더 좌(−)·UW 더 깊으면 = 저금리 유동성 꼬리증폭 (4.2.1 zoom).")
    print("       [B] ramp_up(확대) vs flat·ramp_down: UWmean·UWcvar1 더 깊고 fold 일관 →")
    print("           저금리 하 유동성 *팽창 경로*(debasement)가 꼬리 심화 + 경로순서 효과(저금리에서).")
    print("       term(끝점) 비슷한데 UW만 다르면 → 순수 경로위험.  *상대* 비교만 (절대수치 OOD 한계 §5).")
