"""MTL 3채널 target + bondpp_3m — (excess_liq_wr, sp_return, pp_bond_13w_lag) 동시 학습.
...
target       (3ch × past+future): excess_liq_wr, sp_return, pp_bond_13w_lag (joint AR generation)

--normalize-bondpp: pp_bond_13w_lag 채널만 train mean/std 로 standardize.
                    excess_liq_wr, sp_return 채널은 raw 유지.

비교 대상:
  MTL 3ch (sp+liq+vix):                sp_return median = -2.21 ★
  MTL 2ch + bondpp (sp+bp):            sp_return median = ?
  이 모델 (sp+liq+bp):                  sp_return median = ?
  → vix → bondpp 교체로 OOS 안정성 + sp_return 학습 가치 확인
"""
...
COLS_TARGET = ["excess_liq_wr", "sp_return", "pp_bond_13w_lag"]
COLS_COND   = ["tbill_wr", "tbill_26w_lag", "excess_liq_wr"]
MASK_FUTURE_CH = [1, 2]    # mask tbill_26w_lag + excess_liq_wr in future
