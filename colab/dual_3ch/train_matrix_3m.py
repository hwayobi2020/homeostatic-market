"""3-month cumulative ahead Gaussian forecaster — sp_return future 13w sum 단일 예측.

train_matrix_1step.py 의 horizon 만 1주 → 3개월(13주 누적) 변경. 그 외 동일.

Architecture: CausalTransformer3M (1-step 모델과 동일 구조)
  Encoder: Causal Transformer (d_model=128, n_heads=4, n_layers=3)
  Heads:   분리된 Gaussian (μ, log σ²) head per target channel
  Input:   cond past 52w + target past 52w + cond future 13w (tbill 시나리오)
  Output:  Σ target[t+1..t+13] Gaussian per channel (3M cumulative scalar)

Loss = NLL(sp_3m_sum) + λ_aux × Σ NLL(aux_3m_sum)    (λ_aux = 0.1)
Best ckpt: sp_return val 3M NLL (raw)

Window 길이 L=65 (past 52 + future 13).
ckpt prefix: m3m_v* — 1-step (m1step_v*), multi-step (mtxf_v*, matrix_v*) 와 격리.
SKIP 로직: ckpt + summary 이미 존재 시 학습 건너뜀.

가설 (사용자 통찰):
  weekly 1-step 의 noise 가 큼 → 3M 누적은 √13 ≈ 3.6배 signal-to-noise 비 ↑
  거시 변수 (m2/gdp/cpi) stale forward-fill 영향 자연 평탄화
  → MTL 효과 더 크게 나타날 가능성
"""
...
# =====================================================================
# Model — 1-step 모델과 동일 구조 (encoder 길이만 다름)
# =====================================================================

class CausalTransformer3M(nn.Module):
    """Encoder + scalar Gaussian head per channel (3M cumulative prediction).

    Input:
      cond:        (B, total_len=65, d_cond)              past 52 + future 13
      target_past: (B, past_len=52,  d_target)            past target

    Output:
      heads_out: dict {col: (mu, log_var)} where mu/log_var 는 (B,) — 3M sum prediction
    """
    def __init__(self, d_cond, target_cols, d_model, n_heads, n_layers,
                 past_len, future_len=13, dropout=0.1):
        super().__init__()
    ...
    def forward(self, cond, target_past):
        ...
        # 마지막 step (= future 13w 끝점) → 3M 누적 예측
        h_t13 = h[:, -1, :]

        outputs = {}
        for col in self.target_cols:
            out = self.heads[col](h_t13)
            mu = out[..., 0]
...
def gaussian_nll_scalar(target, mu, log_var):
    """target/mu/log_var: (B,)"""
    return 0.5 * (LOG2PI + log_var + (target - mu).pow(2) * torch.exp(-log_var))


def compute_per_channel_nll(target_3m, outputs, target_cols):
    nll_per_col = {}
    for i, col in enumerate(target_cols):
        mu, log_var = outputs[col]
        nll = gaussian_nll_scalar(target_3m[..., i], mu, log_var)
        nll_per_col[col] = nll.mean()
    return nll_per_col


def total_loss(target_3m, outputs, target_cols, lambda_aux):
    nll_per_col = compute_per_channel_nll(target_3m, outputs, target_cols)
...
def load_windows(csv_path, cols_cond, cols_target, L=65,
                 cond_stats=None, target_stats_map=None, normalize_target_idxs=()):
    """L=65 sliding window — past 52 + future 13."""
...
def compute_3m_target(X, past_len):
    """3M cumulative target — future 13 weeks 의 합. (B, L, D) → (B, D)."""
    return X[:, past_len:, :].sum(dim=1)
...
def run(spec, train_csv, val_csv, test_csv, save_dir, seed,
    ...
    tag_full = f"m3m_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_seed{seed}"
    ...
    print(f"\n{'='*72}")
    print(f"[Variant {spec['variant_id']} = {spec['name']}{norm_tag}{fold_str}] seed={seed}  (3M cumulative Gaussian)")
    print(f"  cond   = {cols_cond}    (D_COND={len(cols_cond)})")
    print(f"  target = {cols_target}  (D_TARGET={len(cols_target)})  [3M sum of future 13w]")
    ...
    def eval_split(X, C):
        ...
        with torch.no_grad():
            ...
            target_3m = compute_3m_target(X, PAST_LEN).to(device)  # (B, d_target)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(0, len(perm), batch):
            ...
            target_3m = compute_3m_target(xb, PAST_LEN)
            outputs = model(cb, target_past)
            loss, _ = total_loss(target_3m, outputs, cols_target, lambda_aux)
            opt.zero_grad()
            loss.backward()
    ...
    if best_state is not None:
        torch.save({
            ...
            "model_type":        "causal_transformer_3m",
            "cond_cols":         cols_cond,
...
def main():
    ap = argparse.ArgumentParser(description="3M cumulative matrix trainer (v33)")
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(
            args.out_dir,
            f"m3m_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_multiseed.csv")
