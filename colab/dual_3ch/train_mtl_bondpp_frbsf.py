"""MTL 3채널 target — (sp_return, pp_bond_13w_lag, frbsf_wr) 동시 학습.
...
target       (3ch × past+future): sp_return, pp_bond_13w_lag, frbsf_wr (joint AR generation)

이 변종 의도:
  - frbsf_wr 는 직교성 검증 통과 (VIF 1.04, weekly nz_diff_pct 100%)
  - target 위치 추가 시 sp_return 학습이 향상되는지 검증 (★ best 위 ablation)
  - cond/target gradient 위치 효과 (bondpp_3m breakthrough) 의 변수 확장

비교 대상:
  ★ MTL 2ch (sp + bondpp):                  sp_return median = -2.199 (현재 best)
    MTL 3ch (sp + liq + bondpp):            sp_return median = -2.16 (negative transfer)
    이 모델 (sp + bondpp + frbsf_wr):        sp_return median = ?

--normalize-bondpp: pp_bond_13w_lag 채널만 train mean/std 로 standardize.
                    sp_return + frbsf_wr 채널은 raw 유지.
"""
...
def load_windows(csv_path, cols_cond, cols_target, L=104,
    ...
    missing = [c for c in cols_target + cols_cond if c not in df.columns]
    if missing:
        raise KeyError(f"CSV {csv_path} 에 누락 컬럼: {missing}. data/build_frbsf_weekly.py 실행 필요.")
...
def run(train_csv, test_csv, save_dir, seed=42, normalize_bondpp=False):
    ...
    print(f"\n{'=' * 70}")
    print(f"[MTL_bp_frbsf{tag_norm}] seed={seed}")
    ...
    tag = f"mtl_bp_frbsf{tag_norm}_seed{seed}"
...
def main():
    ...
    ap.add_argument("--train-csv", default=os.path.join(HERE, "data", "weekly_ppbond_frbsf_train.csv"))
    ap.add_argument("--test-csv",  default=os.path.join(HERE, "data", "weekly_ppbond_frbsf_test.csv"))
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(args.out_dir, f"mtl_bp_frbsf{tag_norm}_multiseed_results.csv")
        df.to_csv(out_csv, index=False)
        print(f"\n[MTL_bp_frbsf{tag_norm}] multi-seed (n={len(df)})")
        ...
        print(f"\n  ★ best (sp+bondpp 2ch) sp_return median=-2.199. this sp_return median={df['test_sp_return'].median():+.4f}")
