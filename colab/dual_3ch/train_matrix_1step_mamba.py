...
ckpt prefix: m1step_mamba_v* — matrix multi-step (matrix_v*, mtxf_v*) 와 격리.
...
class Mamba1Step(nn.Module):
    ...
    def __init__(self, d_cond, target_cols, d_model, n_heads, n_layers,
        ...
        # Mamba — selective state-space, 자체 causal, O(L)
    ...
    def forward(self, cond, target_past):
        ...
        # Mamba encoder (자체 causal, no mask/pos_emb)
        h = self.encoder(x)
...
def run(spec, train_csv, val_csv, test_csv, save_dir, seed,
    ...
    tag_full = f"m1step_mamba_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_seed{seed}"
...
def main():
    ...
    if len(results) > 1:
        ...
        out_csv = os.path.join(
            args.out_dir,
            f"m1step_mamba_v{spec['variant_id']}_{spec['name']}{norm_tag}{fold_str}_multiseed.csv")
