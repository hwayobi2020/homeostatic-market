...
CACHE_DIR = os.path.join(PS.RESULT_DIR, "pathshape_joint_ratestep_metabmm_k1_cache")
...
def main():
    ...
    print(f"#   금리=상승 step(음수 없음, in-dist) 고정 배경, 유동성=평균분산 일치 4종.  k=1.  지표 skew/uw_mean/uw_cvar1")
    print("#" * 114)
...
def _summarize():
    print("\n" + "=" * 114)
    print("[집계] fold × seed 평균  (값 = skew / uw_mean / uw_cvar1)")
    for fold in FOLDS:
        ...
        for tag, key in (("flat (양축 지속)", "flat"), ("rate_only (금리 step↑ 단독)", "rate_only")):
            ...
            uc = _agg([d[key]["uw_cvar1"] for d in loaded])
            print(f"  {tag:<26} skew={sk:+.3f}  uw_mean={um:+.4f}  uw_cvar1={uc:+.4f}")
        print(f"  [금리 step↑ + 유동성 (평균분산 일치)]")
        print(f"    {'시나리오':<16} {'skew':>9} {'uw_mean':>10} {'uw_cvar1':>10}")
        ...
        for a, b, tag in (("유동성 ramp↑", "유동성 ramp↓", "ramp ↑vs↓"),
            ...
            ua = _agg([d["joint"][a]["uw_cvar1"] for d in loaded])
            ub = _agg([d["joint"][b]["uw_cvar1"] for d in loaded])
            deeper = a if ua < ub else b
            print(f"    [{tag}, uw_cvar1] {a}={ua:+.4f} vs {b}={ub:+.4f} → {deeper} 더 깊음")
