...
인덱스도 정확히 맞는다.  첫 생성 주는 원점 다음 주 o+1 이고, 그 학습 σ 는
`sp_std_13w[o+1] = std(r_{o-12..o})` 다.  여기 첫 창 `tail = raw[o-12 : o+1]`
이 바로 그 13 개다.  (한때 이 도크스트링에 "원점 포함이라 한 주 어긋난다"고
적었으나 **틀린 서술이었다** — 되돌렸다.)

역변환은 손실·체크포인트 선택(둘 다 z 공간)에 들어가지 않으므로 이 비교는 기존
가중치로 성립하고, 같은 설정 재학습은 역변환 질문에 답하지 못한다.  다만 GPU
학습이 결정적이지 않아(`major revision/실험계획.md` 미결 4번:
cudnn.deterministic 미설정, 같은 시드 CRPS 10^-4 드리프트) "재학습하면 글자
그대로 같은 가중치"라고 말할 수도 없다 — 그건 재현성 검증이지 이 질문의 답이
아니다.

가중치가 실제로 달라지는 유일한 변형은 **손실·체크포인트 선택을 롤링 역변환 후
raw 공간으로 옮기는 것**이다 (raw CRPS 는 원점을 σ_i 로 가중하고, raw NLL 에는
Σ log σ_h 야코비안이 붙는데 롤링 σ 는 생성 경로에 의존해 상수가 아니다).
그건 새 설정이고 이 스크립트의 범위 밖이다.
