"""Monthly 1-step matrix batch runner — 9 variants × 3 folds × N seeds.

train_matrix_monthly.py 호출 자동화. 변종별 정규화 옵션 자동 적용.
SKIP 로직 동일.

Usage (Colab T4):
  cd /content/drive/MyDrive/Colab Notebooks/homeostatic-market
  python colab/run_matrix_monthly.py --variants 1 2 3 4 5 6 7 8 9 \
                                      --folds F1 F2 F3 \
...
TRAIN_SCRIPT = os.path.join(HERE, "dual_3ch", "train_matrix_monthly.py")
...
def main():
    ...
    print(f"\n{'#' * 72}")
    print(f"# Monthly 1-step matrix batch (Causal Transformer + 1-month Gaussian head)")
