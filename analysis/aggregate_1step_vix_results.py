"""1-step matrix (vix included) 결과 분석 — m1step_vix_v* prefix only.
...
def main():
    ...
    folds_dir  = args.folds_dir  or os.path.join(args.root, "data", "folds_v33_vix")
    ...
    # === 1-step results (메인) ===
    s1_recs = load_summaries(result_dir, "m1step_vix_v*_summary.json")
    print(f"\n1-step (m1step_vix_v*) summaries: {len(s1_recs)}  (expected 135 = 9×3×5)")
    if not s1_recs:
    ...
    out_csv = os.path.join(result_dir, "1step_vix_results.csv")
