"""1-step matrix 결과 분석 — m1step_v* prefix only.
...
def main():
    ...
    # === 1-step results (메인) ===
    s1_recs = load_summaries(result_dir, "m1step_v*_summary.json")
    print(f"\n1-step (m1step_v*) summaries: {len(s1_recs)}  (expected 135 = 9×3×5)")
    if not s1_recs:
    ...
    out_csv = os.path.join(result_dir, "1step_results.csv")
