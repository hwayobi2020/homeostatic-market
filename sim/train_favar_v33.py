"""FAVAR v33 prototype 학습 — joint generation 구조, NLL only, 빠른 prototype.

설계 (v31/phase2 baseline 의 축소 변종):
    Conditioning C [B, L, 3]  : (m2_growth, m2v, cpi_yoy)  ← 외생 시나리오 입력
    Target       X [B, L, 2]  : (sp_return, margin_chg)    ← 공동 생성 출력
    K = 2 causal affine steps (was 3)
    L = 52, PAST_LEN = 26  (was 208/156 in v4. prototype 단계 축소)
...
산출:
    models/favar_v33_proto_best.pt
    result/favar_v33_proto_trainlog.csv
...
REPO       = HERE.parent
TRAIN_CSV  = REPO / 'data' / 'weekly_v33_train.csv'
TEST_CSV   = REPO / 'data' / 'weekly_v33_test.csv'
...
def main():
    ...
    ap.add_argument('--lambda-phys', type=float, default=0.0,    help='PINN sign-match penalty weight')
    ap.add_argument('--penalty-type', type=str, default='relu',
                    choices=['relu', 'tanh', 'hinge', 'corr'],
                    help='PINN penalty function form')
    ap.add_argument('--architecture', type=str, default='causal',
                    choices=['causal', 'coupling'],
                    help='causal: 기존 affine causal flow (channel independent). '
                         'coupling: RealNVP cross-channel coupling (channel dependent)')
    ap.add_argument('--use-wavelet', action='store_true',
                    help='param_net 와 param_head 사이에 Mexican Hat wavelet activation 삽입 '
                         '(compact support 로 mean shift 차단)')
    ...
    ckpt_path = ckpt_dir / f'favar_v33_{args.tag}_best.pt'
    log_path  = log_dir  / f'favar_v33_{args.tag}_trainlog.csv'
