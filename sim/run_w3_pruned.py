"""W3: pruned features (13dim) based on importance analysis"""
...
class ExpertEnv(gym.Env):
    def __init__(self, config=None):
        ...
        # 13dim: pp-hwm, pp-1, ndx_1m, sp_1m, vix, 52w_6개, last_action, pp/hwm
...
def full_metrics(rets, name):
    ...
    print(f"{name:>22}: ret={ann:+6.2f}% vol={vol:5.1f}% sharpe={sharpe:+5.2f} sortino={sortino:+5.2f} mdd={mdd:5.1f}%", flush=True)
...
if __name__ == "__main__":
    ...
    for wname, test_start, test_end in windows:
        ...
        print("=" * 70, flush=True)
        print(f"{wname}: Train ~{test_start[:4]} (13dim pruned: ndx_1m, sp_1m, vix, 52w*6, state*4)", flush=True)
        ...
        model.save(f"models/equanimity_{wname.lower()}_13dim_pruned")
    ...
    print("Reference (19dim):", flush=True)
    print("  W1: Sharpe=0.99 Return=+12.5% MDD=-17.0%", flush=True)
    print("  W2: Sharpe=0.62 Return=+6.3%  MDD=-15.4%", flush=True)
    print("  W3: Sharpe=0.90 Return=+12.7% MDD=-13.3%", flush=True)
    print("Reference (13dim original, no 52w):", flush=True)
    print("  W1: Sharpe=1.12 W2: 0.62 W3: 0.87", flush=True)
