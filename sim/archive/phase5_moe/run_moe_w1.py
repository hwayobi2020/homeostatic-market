"""
MoE W1: Expert1(no leverage) + Expert2(2x leverage) + Router(discrete, return reward)
"""
...
if __name__ == "__main__":
    ...
    print("Experts loaded.", flush=True)
    ...
    router.save("models/router_w1_discrete")
    ...
    print(f"Final W: mean={acts.mean():.2f} min={acts.min():.2f} max={acts.max():.2f}", flush=True)

    dates = pd.to_datetime(test_data["date"].values[:len(acts)])
    print("\nmonthly:", flush=True)
    for i in range(len(acts)):
        d = f"{dates[i].year}-{dates[i].month:02d}"
        ex = "E1" if choices[i] == 0 else "E2"
        print(f"  {d} {ex} e1={e1_acts[i]:.2f} e2={e2_acts[i]:.2f} -> w={acts[i]:.2f}", flush=True)
